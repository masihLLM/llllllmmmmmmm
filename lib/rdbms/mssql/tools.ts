...
export const mssqlTools = {
  listSchemasMssql: tool({
    description: "List available schemas in the connected MSSQL database.",
  ...
  listTablesMssql: tool({
    description: "List tables in MSSQL. If schema is omitted, list all schemas' tables.",
  ...
  listColumnsMssql: tool({
    description: "List columns for a given table in a schema in MSSQL.",
  ...
  runReadOnlySQLMssql: tool({
    description: "Execute a read-only SQL query (SELECT/CTE) on MSSQL. Returns JSON rows.",
